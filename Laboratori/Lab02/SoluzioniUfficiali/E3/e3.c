...
typedef struct {
  /* modifica rispetto all'es. 4 del Lab01*/
  char *codice;
  char *partenza;
  char *destinazione;
  char *data_str;
...
int main(int argc, char** argv) {
  ...
  do {
    ...
    case 8: {
      ...
      if (trovato)
        stampaEntry(tab->logC[indice], stdout);
    }
    break;
...
void liberaTabella(tabella_t *tab) {
	...
	if (tab->log) {
		for(i=0;i<tab->n_voci;i++) {
			if (tab->log[i].codice)
			   free(tab->log[i].codice);
			if (tab->log[i].partenza)
			   free(tab->log[i].partenza);
			if (tab->log[i].destinazione)
			   free(tab->log[i].destinazione);
			if (tab->log[i].data_str)
			   free(tab->log[i].data_str);
			if (tab->log[i].orap_str)
			   free(tab->log[i].orap_str);
			if (tab->log[i].orad_str)
			   free(tab->log[i].orad_str);
		}
		free(tab->log);
	}
	free(tab);
...
int cerca(tabella_t *tab, int start, chiaveOrdinamento k) {
  ...
  for(j=start+1; j<tab->n_voci; j++) {
    switch(k) {
    case DATA: {
      cmp = comparaData(tab->log[min].data, tab->log[j].data);
      if (cmp == 0)
        if(comparaOra(tab->log[min].p, tab->log[j].p) > 0)
          min = j;
    }
    break;
    case CODICE: {
      if(strcmp(tab->log[min].codice, tab->log[j].codice) > 0)
        min = j;
    }
    break;
    case PARTENZA: {
      if(strcmp(tab->log[min].partenza, tab->log[j].partenza) > 0)
        min = j;
    }
    break;
    default: {
...
int confrontaVoci(voce_t *v1, voce_t *v2, chiaveOrdinamento k) {
  ...
  switch(k) {
    case DATA: {
      cmp = comparaData(v1->data, v2->data);
      if (cmp == 0)
        return comparaOra(v1->p, v2->p);
      return cmp;
    }
    break;
    case CODICE: {
...
bool ricercaCodiceDicotomica(tabella_t *tab, char* codice, int *indice) {
  int l = 0, r = tab->n_voci - 1, m, trovato = 0, cmp;

  while(l <= r && trovato == 0) {
    m = (l + r) / 2;
    cmp = strcmp(tab->logC[m]->codice, codice);
    if(cmp == 0) {
      *indice = m;
      trovato = 1;
    }
...
void ricercaPartenzaDicotomica(tabella_t *tab, char *partenza, int nCharConfronto) {
  int l = 0, r = tab->n_voci - 1, m, i, j, cmp;
  bool trovato = 0;

  while(l <= r&& !trovato) {
    m = (l + r) / 2;
    cmp = strncmp(tab->logP[m]->partenza, partenza, nCharConfronto);
    if(cmp == 0) {
      trovato = 1;
    } else {
      if(cmp < 0)
