...
int cerca(voce_t log[MAXR], int n_voci, int start, chiaveOrdinamento k) {
  ...
  for(j=start+1; j<n_voci; j++) {
    switch(k) {
    case DATA: {
      cmp = comparaData(log[min].data, log[j].data);
      if (cmp == 0)
        if(comparaOra(log[min].p, log[j].p) > 0)
          min = j;
    }
    break;
    case CODICE: {
      if(strcmp(log[min].codice, log[j].codice) > 0)
        min = j;
    }
    break;
    case PARTENZA: {
      if(strcmp(log[min].partenza, log[j].partenza) > 0)
        min = j;
    }
    break;
    default: {
...
bool ricercaCodiceLineare(voce_t log[MAXR], int n_voci, char* codice, int *indice) {
  ...
  while(i<n_voci && trovato == 0)
    if (strcmp(codice, log[i].codice)==0){
      trovato = 1;
      *indice = i;
    }
    else
...
bool ricercaCodiceDicotomica(voce_t log[MAXR], int n_voci, char* codice, int *indice) {
  int l = 0, r = n_voci - 1, m, trovato = 0, cmp;

  while(l <= r && trovato == 0) {
    m = (l + r) / 2;
    cmp = strcmp(log[m].codice, codice);
    if(cmp == 0) {
      *indice = m;
      trovato = 1;
    }
...
void ricercaPartenzaDicotomica(voce_t log[MAXR], int n_voci, char *partenza, int nCharConfronto) {
  int l = 0, r = n_voci - 1, m, i, j, cmp;
  bool trovato = 0;

  while(l <= r&& !trovato) {
    m = (l + r) / 2;
    cmp = strncmp(log[m].partenza, partenza, nCharConfronto);
    if(cmp == 0) {
      trovato = 1;
    } else {
      if(cmp < 0)
