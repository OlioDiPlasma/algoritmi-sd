...
int main(int argc, char** argv) {
  ...
  do {
    ...
    case 7: {
      ...
      if (trovato)
        stampaEntry(tab.log[indice], stdout);
      else
...
bool ricercaCodiceLineare(tabella_t tab, char* codice, int *indice) {
  ...
  while(i<tab.n_voci && trovato == 0)
    if (strcmp(codice, tab.log[i].codice)==0){
      trovato = 1;
      *indice = i;
    }
    else
...
bool ricercaCodiceDicotomica(tabella_t tab, char* codice, int *indice) {
  ...
  while(l <= r && trovato == 0) {
    m = (l + r) / 2;
    cmp = strcmp(tab.log[m].codice, codice);
    if(cmp == 0) {
      *indice = m;
      trovato = 1;
    }
...
void ricercaPartenzaDicotomica(tabella_t tab, char *partenza, int nCharConfronto) {
  ...
  while(l <= r && !trovato) {
    m = (l + r) / 2;
    cmp = strncmp(tab.log[m].partenza, partenza, nCharConfronto);
    if(cmp == 0) {
      trovato = 1;
    } else {
      if(cmp < 0)
